...
import           Data.Aeson                              (FromJSON, Result (..), ToJSON, encode, fromJSON, Value, parseJSON)
...
type ContractLog = WLog.WalletLog (Either Text UserContractState)
...
altswapUserContract altswap =
    fmap Map.fromList $ forM knownWallets $ \w -> do
    cid <- Simulator.activateContract w $ AltSwapUser altswap
    ...
    -- v <- flip Simulator.waitForState cid $ \json -> case (fromJSON json :: Result (Monoid.Last (Either Text UserContractState))) of
            ...
            -- _                                                      -> Nothing
    v <- getState (contractState "funds") cid
    ...
    where
        ...
        getState :: (FromJSON a) => (a -> b) -> ContractInstanceId -> Simulator.Simulation t b
        getState f = Simulator.waitForState (fmap f . fromJSONValue)

        contractState :: Text -> ContractLog -> Maybe UserContractState
        contractState logKey (WLog.lookup logKey -> Just (Right v)) = Just v
