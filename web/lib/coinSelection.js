...
const CoinSelection = {
  ...
  randomImprove: (inputs, outputs, limit, preset = []) => {
    ...
    let amount = _wasm.Value.new(_wasm.BigNum.from_str("0"));
    ...
    for (let i = 0; i < splitOutputsAmounts.length; i++) {
      ...
      let range = {};
      range.ideal = _wasm.Value.new(_wasm.BigNum.from_str("0"))
        .checked_add(splitOutputsAmounts[i])
        .checked_add(splitOutputsAmounts[i]);
      range.maximum = _wasm.Value.new(_wasm.BigNum.from_str("0"))
    ...
    if (utxoSelection.remaining.length > 0) {
      ...
      let minAmount = _wasm.Value.new(
        _wasm.min_ada_required(
          change,
          true,
          _wasm.BigNum.from_str(protocolParameters.minUTxO)
        )
      );
      ...
      maxFee = _wasm.Value.new(
        _wasm.BigNum.from_str(maxFee.toString())
      );
      ...
      if (compare(change, minAmount) < 0) {
        ...
        const minAda = minAmount
          .checked_sub(_wasm.Value.new(change.coin()))
          .checked_add(_wasm.Value.new(utxoSelection.amount.coin()));
...
function improve(utxoSelection, outputAmount, limit, range) {
  ...
  const newAmount = _wasm.Value.new(_wasm.BigNum.from_str("0"))
...
function mergeOutputsAmounts(outputs) {
  let compiledAmountList = _wasm.Value.new(_wasm.BigNum.from_str("0"));
...
function splitAmounts(amounts) {
  ...
  if (amounts.multiasset()) {
    ...
    for (let i = 0; i < mA.keys().len(); i++) {
      ...
      for (let j = 0; j < mA.get(scriptHash).keys().len(); j++) {
        let _assets = _wasm.Assets.new();
        let assetName = mA.get(scriptHash).keys().get(j);

        _assets.insert(
          _wasm.AssetName.from_bytes(assetName.to_bytes()),
          _wasm.BigNum.from_bytes(
            mA.get(scriptHash).get(assetName).to_bytes()
          )
        );

        let _multiasset = _wasm.MultiAsset.new();
        _multiasset.insert(
          _wasm.ScriptHash.from_bytes(scriptHash.to_bytes()),
          _assets
        );
        let _value = _wasm.Value.new(_wasm.BigNum.from_str("0"));
  ...
  splitAmounts.push(
    _wasm.Value.new(_wasm.BigNum.from_bytes(amounts.coin().to_bytes()))
  );
  ...
  if (minUTxOValue && BigInt(outputAmount.coin().to_str()) > 0) {
    let minAmount = _wasm.Value.new(
      _wasm.min_ada_required(
        cumulatedAmount,
        true,
        _wasm.BigNum.from_str(minUTxOValue.toString())
      )
    );
...
const cloneUTxOList = (utxoList) =>
  utxoList.map((utxo) =>
    _wasm.TransactionUnspentOutput.from_bytes(utxo.to_bytes())
  );
 ...
 */
const cloneValue = (value) => _wasm.Value.from_bytes(value.to_bytes());
